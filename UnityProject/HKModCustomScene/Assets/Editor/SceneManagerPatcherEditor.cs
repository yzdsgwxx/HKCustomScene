...
//
// ⚠ 依赖 SFCore.dll：这个文件 using 了 SFCore.MonoBehaviours。
//    如果 Assets/Assemblies/SFCore.dll 还不在，Unity 会报
//    "The type or namespace name 'SFCore' could not be found"，并且整个 Editor 程序集都编译不过。
//    → 所以务必**先把 SFCore.dll 放进 Assets/Assemblies/，再拷入本文件**。
//
// 作用：给 SFCore 的 SceneManagerPatcher 做一个好用的 Inspector：
//       把音乐/环境音/气氛等的“索引”变成可读的下拉列表，并自动写回对应的名字字段。
using UnityEngine;
